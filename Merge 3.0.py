...
word_result = Workbook(write_only=True)
#wrs - word result sheet 
wrs = word_result.create_sheet()

def create_header(result_table, reporter):
    #create header потом допилить для экспорта
    if (format == 'import'):
        n1 = reporter +' imp from partn, $1000, 20'

        wrs.append(['Reporter', 'Partner','Product code', 'Product label', n1 + str(i) for i in range(20)])
