...
#load 
wbd= load_workbook('C:/GitHub/ITC Upgrade Test/Tables 25/Imports/Israel/Trade_Map_-_Existing_and_potential_trade_between_Israel_and_Afghanistan.xlsx')
wsd = wbd.active
...
for product in range(productCount):
    wrs['C' + str(product + 2)] = wsd['A'+str(14+product)].value[1:]
    wrs['D' + str(product + 2)] = wsd['B'+str(14+product)].value

for row in range(productCount):
    for column1 in range(20):
        wrs.cell(row=row+2, column=5+column1).value = wsd.cell(row=row+14, column=3+column1).value
    for column2 in range(20):
        wrs.cell(row=row+2, column=25+column2).value = wsd.cell(row=row+14, column=24+column2).value
    for column2 in range(20):
        wrs.cell(row=row+2, column=45+column2).value = wsd.cell(row=row+14, column=45+column2).value
