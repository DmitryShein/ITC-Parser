...
def Open_link(reporter, partner):

    #open link
    driver.get('https://www.trademap.org/Bilateral_TS.aspx?nvpm=1%7c'+reporter+'%7c%7c'+partner+'%7c%7cTOTAL%7c%7c%7c4%7c1%7c1%7c1%7c2%7c1%7c1%7c1%7c1%7c1')
    print('Сделали запросик на reporter - ',reporter,'pather = ',partner)
    #choose 20 per-age
    select1 = Select(driver.find_element("name", "ctl00$PageContent$GridViewPanelControl$DropDownList_NumTimePeriod"))
    select1.select_by_visible_text("20 per page")

    #choose 6d cluster 5-6k of products
    #that workin only for autorizate accaounts!
    select2 = Select(driver.find_element("name", "ctl00$NavigationControl$DropDownList_ProductClusterLevel"))
    select2.select_by_visible_text("Product cluster at 6 digits")

    #wait for loading full page
...
for part in partArr:
    ...
    if (part != '376'):
        try:
            ...
            with open("C:/GitHub/ITC Upgrade Test/Errors.txt", "w") as f:   # Opens file and casts as f 
                f.write("Ошибка возникла в паре: r - 376, partner -" + part + '; ')
